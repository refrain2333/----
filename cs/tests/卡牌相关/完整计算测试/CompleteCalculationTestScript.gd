...
func _create_simple_game_manager():
	...
	const SimpleTestGameManagerClass = preload("res://cs/tests/卡牌相关/牌型识别测试/SimpleGameManager.gd")
	game_manager = SimpleTestGameManagerClass.new()
...
func _analyze_hand_type(cards: Array) -> Dictionary:
	...
	# 使用V2.3结果
	var final_result = _create_analysis_result_from_v2(v2_result, card_data_array)
...
## 🔧 从V2.3结果创建分析结果
func _create_analysis_result_from_v2(v2_result, card_data_array: Array) -> Dictionary:
	# 如果V2.3结果可用，使用它
...
func _update_hand_type_display(result: Dictionary):
	...
	if best_cards_label:
		# V2.3增强显示：使用HandResult的格式化方法
		if result.get("version", "") == "V2.3" and result.has("hand_result"):
			...
			if hand_result and hand_result.has_method("format_display"):
				...
				# 备用显示方式
				best_cards_label.text = "V2.3结果: %s" % result.get("hand_type_name", "未知")
		else:
			# 兼容显示
			var cards_text = ""
	...
	if result.get("version", "") == "V2.3":
		...
		# 显示简化公式
		calc_text = "计算: %s" % result.get("detailed_formula", result.get("calculation_formula", "无"))
...
func _display_contributing_cards(result: Dictionary):
	...
	if result.get("version", "") == "V2.3" and result.has("hand_result"):
		var hand_result = result.hand_result
		print("HandTypeTest: V2.3结果，hand_result存在: %s" % (hand_result != null))
		if hand_result and hand_result.contributing_cards:
			contributing_cards = hand_result.contributing_cards
			print("HandTypeTest: 找到V2.3 contributing_cards: %d张" % contributing_cards.size())
		else:
			print("HandTypeTest: V2.3 hand_result中没有contributing_cards或为空")
	else:
		# 兼容：使用best_hand_cards
		contributing_cards = result.get("best_hand_cards", [])
		print("HandTypeTest: 使用兼容模式，best_hand_cards: %d张" % contributing_cards.size())
...
func _run_v2_test_suite():
	# 测试1：基础功能测试
	print("🧪 V2.3测试1: 基础功能测试")
	_run_v2_basic_tests()

	# 测试2：性能基准测试
	print("🧪 V2.3测试2: 性能基准测试")
